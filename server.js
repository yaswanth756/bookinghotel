...
app.get("/user/booking/payment/:id", (req, res) => {
    ...
    const sql = `
        SELECT b.*, r.description AS room_description 
        FROM Bookings b
        JOIN Rooms r ON b.room_id = r.room_id
        WHERE b.user_id = ? AND b.status != 'cancelled'`;  // Added quotes around 'cancelled'

    db.query(sql, [userId], (err, results) => {
        if (err) {
            console.error("Error retrieving booking:", err);
            return res.status(500).json({ message: "Error retrievering booking details." });
        }
        console.log(results.length);
        if (results.length === 0) {
            return res.status(200).json({ message: "No booking found for this user." });
        }
...
app.delete('/booking/cancel', (req, res) => {
    ...
    const cancelBookingQuery = `
        ...
        WHERE user_id = ? AND room_id = ? AND status = 'booked';
    `;

    // Update booking status to 'cancelled'
    db.query(cancelBookingQuery, [user_id, room_id], (err, result) => {
        if (err) {
            console.error(err);
        ...
        if (result.affectedRows === 0) {
            return res.status(404).json({ message: "No active booking found for the given user_id and room_id." });
        }
...
app.get('/booking/history/:id', (req, res) => {
    ...
    const userId = req.params.id;
    const sql = 'SELECT room_type,created_at,payment FROM Bookings WHERE user_id = ? AND status != "cancelled "';
